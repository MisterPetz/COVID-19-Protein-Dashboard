...
source("plot_spike_domains.R")
source("info_observe.R")


if (!requireNamespace("pacman", quietly = TRUE)) install.packages("pacman")
pacman::p_load(shiny, shinydashboard, bio3d, NGLVieweR, rgl, Rvcg, shinycssloaders, assertthat,  sf, leaflet, maps,rnaturalearth, dplyr, ggplot2)

working_dir <- "/home/johannes/University/Master_Module_plus/Fertig/FAllStudie"
setwd(working_dir)
...
addCustomColoring <- function(viewer, isPreloaded) {
  if (isPreloaded) {
    # Predefined residue ranges and colors for preloaded PDB
    ...
    for (i in seq_along(begin)) {
      viewer <- viewer %>%
        addRepresentation(
          "cartoon",
          param = list(
            color = col[i],                    # Custom color
            sele = paste0(begin[i], "-", end[i])  # Residue range (e.g., "15-305")
          )
        )
    }
  } else {
    # Default coloring for fetched PDB
    viewer <- viewer %>%
      addRepresentation("cartoon", param = list(colorScheme = "chainIndex"))
  }
...
ui <- dashboardPage(
dashboardHeader(
  title = HTML("Pandemic in Data: Biochemical Insights into SARS-CoV-2"),
  disable = FALSE,
  titleWidth = 558
),
...
dashboardSidebar(
  sidebarMenu(
    id = "tabs",
    menuItem("Welcome Page", tabName = "welcome", icon = icon("door-open")),
    menuItem("SARS-CoV-2 Structure", tabName = "structure", icon = icon("virus-covid")),
    menuItem("Pandemic Data", tabName= "map", icon=icon("chart-line"))
  )
),
...
dashboardBody(
  ...
  tabItems(
    ...
    tabItem(
      tabName = "welcome",
      div(id = "welcome-page",  
          htmltools::includeHTML("./files/Text/Welcome.html")
          ),
      
      div(
        class = "button-container",
        actionButton("gotoStructure", "Go to Structure Data", icon = icon("cube"), class = "btn-primary"),
        actionButton("gotoEnvelope", "Go to Envelope Virus Surface", icon = icon("virus"), class = "btn-success"),
        actionButton("gotoMap", "Go to Map and Corona Data", icon = icon("globe"), class = "btn-secondary")
      ),
      div(
        class = "footer-attribution",
        "Made with ❤️ by Johannes Peterlechner"
      )
      ),
    # Structure Page
    tabItem(
      ...
      fluidRow(
        column(
          ...
          tabsetPanel(
            ...
            tabPanel(
              ...
              fluidRow(
                column(
                  width = 6,
                  box(
                    title = tagList("Sars-Cov19 Spike Protein Viewer", infoButtonModuleUI("protein_structure_info")), width = NULL, solidHeader = TRUE,
                    status = "primary", background = "black",
                    withSpinner(NGLVieweROutput("structureViewer", width = "700px", height = "600px"))
                  ),
                  
                  box(
                    title = tagList("Select Representation Type:", infoButtonModuleUI("representation_info")),
                    width = NULL, solidHeader = TRUE, status = "info",
                    radioButtons(
                      "representationType", label = h3(""),
                      choices = c("cartoon", "ball+stick", "spacefill", "licorice", "backbone"),
                      selected = "cartoon", inline = TRUE
                    ),
                    actionButton("loadRepresentation", "Load Representation"),
                    actionButton("resetView", "Reset on Spike Protein")
                  ),
                  
                  box(
                    title = tagList("Fetch PDB File:", infoButtonModuleUI("pdb_info")),
                    width = NULL, solidHeader = TRUE, status = "info",
                    textInput("pdbID", "Enter PDB ID:", placeholder = "e.g., 1BXN"),
                    actionButton("fetchBtn", "Fetch PDB"),
                    verbatimTextOutput("fetchStatus")
                  )
                ),
                column(
                  width = 6,
                  box(
                    title = tagList("Domains: Important Spike-Protein Domains", infoButtonModuleUI("domain_info")), 
                    width = NULL, solidHeader = TRUE,
                    status = "primary",
                    plotOutput("domainPlot", height = "250px"),
                    div(
                      style = "background-color: #f9f9f9; border-radius: 10px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); margin-top: 10px;",
                      h3(style = "text-align: center; color: #007BFF; font-weight: bold;", "Explanation of Spike Protein Domains"),
                      div(
                        style = "display: flex; flex-wrap: wrap; gap: 20px; justify-content: space-between;",
                        div(
                          style = "flex: 1 1 calc(50% - 10px); background-color: #96ceb4; border-radius: 8px; padding: 15px;",
                          tags$b("N-Terminal Domain (NTD):"),
                          p("Contributes to immune evasion, important in human immune response.")
                        ),
                        
                        div(
                          style = "flex: 1 1 calc(50% - 10px); background-color: #ff6f69; border-radius: 8px; padding: 15px;",
                          tags$b("Receptor-Binding Domain (RBD):"),
                          p("Facilitates binding to the human hACE2 receptor, critical for viral entry. Enables Virus to enter Human cell. ")
                        ),
                        
                        div(
                          style = "flex: 1 1 calc(50% - 10px); background-color: #ffcc5c; border-radius: 8px; padding: 15px;",
                          tags$b("Fusion Peptide (FP):"),
                          p("Inserts into the host membrane to destabilize it, enabling membrane fusion.")
                        ),
                        
                        div(
                          style = "flex: 1 1 calc(50% - 10px); background-color: #f8d7da; border-radius: 8px; padding: 15px;",
                          tags$b("Heptad Repeats (HR1 and HR2):"),
                          p("Drive membrane fusion by forming a six-helix bundle.")
                        ),
                        
                        div(
                          style = "flex: 1 1 calc(50% - 10px); background-color: #d1ecf1; border-radius: 8px; padding: 15px;",
                          tags$b("Transmembrane Domain (TM):"),
                          p("Anchors the spike protein to the viral envelope.")
                        )
                      )
                    )
                  )
            ...
            # Envelope Virus Surface Tab
            tabPanel(
              "Envelope Virus Surface",
              fluidRow(
                column(
                  width = 6,
                  box(
                    title = tagList("Virus Surface", infoButtonModuleUI("envelope_view_info")), solidHeader = TRUE,
                    status = "primary", width = NULL, background = "black",
                    withSpinner(rglwidgetOutput("modelViewer", width = "700px", height = "600px"))
                  )
                ),
                column(
                  width = 6,
                  box(
                    title = "Information", width = NULL, solidHeader = TRUE,
                    status = "info",
                    uiOutput("modelInfo")
                  )
                )
...
infoButtonModule("representation_info", "representation")
infoButtonModule("pdb_info", "pdb")
infoButtonModule("protein_structure_info", "protein_structure")
infoButtonModule("envelope_view_info", "envelope_view")
infoButtonModule("domain_info", "domain")

pdbPath <- reactiveVal(initial_pdb_path)
isPreloaded <- reactiveVal(TRUE)
plyPath <- file.path(getwd(), "files/SARSCOV2_Thick.ply")
assert_that(file.exists(plyPath), msg = paste("ply file not found:", plyPath))

#observe switch to structure page button
observeEvent(input$gotoStructure, {
  updateTabItems(session, "tabs", selected = "structure")
})
observeEvent(input$gotoEnvelope, {
  updateTabItems(session, "tabs", selected = "structure")  # Switch to the structure sidebar tab
  updateTabsetPanel(session, "structureTabs", selected = "Envelope Virus Surface")  # Switch to the Envelope Virus tabPanel
})
...
# Fetch a new PDB file
observeEvent(input$fetchBtn, {
  req(input$pdbID)
  pdbID <- tolower(trimws(input$pdbID))
  output$fetchStatus <- renderText({ "Fetching PDB file..." })
  
  tryCatch({
    filePath <- get.pdb(pdbID, path = tempdir(), overwrite = TRUE)
    pdbPath(filePath)
    isPreloaded(FALSE)  # Mark as fetched PDB
    pdb <- read.pdb(filePath)
    summaryText <- capture.output(summary(pdb))
    output$pdbSummary <- renderText({
      paste(summaryText, collapse = "\n")
    })
    output$fetchStatus <- renderText({ paste("Successfully fetched PDB ID:", pdbID) })
  }, error = function(e) {
    output$fetchStatus <- renderText({ paste("Error fetching PDB ID:", pdbID) })
  })
})

# Render the structure viewer
output$structureViewer <- renderNGLVieweR({
  req(pdbPath())  
  
  # Initialize NGLVieweR with the PDB path
  ngl_viewer <- NGLVieweR(pdbPath())
  
  # Apply custom coloring based on preloaded status
  addCustomColoring(ngl_viewer, isPreloaded())
})

# Load a specific representation
observeEvent(input$loadRepresentation, {
  req(pdbPath(), input$representationType) 
  
  output$structureViewer <- renderNGLVieweR({
    # Initialize NGLVieweR with the PDB path
    ngl_viewer <- NGLVieweR(pdbPath()) %>%
      addRepresentation(input$representationType, param = list(colorScheme = "chainIndex"))
...
observeEvent(input$resetView, {
  # Reset the PDB path to the preloaded file
  pdbPath(initial_pdb_path)
  isPreloaded(TRUE)  
  
  output$structureViewer <- renderNGLVieweR({
    # Initialize NGLVieweR with the preloaded PDB path
    ngl_viewer <- NGLVieweR(pdbPath())
    
    # Apply custom coloring for the preloaded PDB
    addCustomColoring(ngl_viewer, isPreloaded()) %>%
      setFocus()
  })
})
...
output$modelInfo <- renderUI({
  tags$div(
    tags$h3("What are the parts of a coronavirus?"),
    tags$p("Coronaviruses are a large family of viruses, some of which infect humans. 
            The coronavirus at the root of COVID-19 is the newest known member of this family. 
            Like other coronaviruses that infect people, it causes respiratory disease, among other symptoms."),
    tags$ul(
      tags$li(
        tags$strong("RNA"), 
        " At their core, coronaviruses contain a genetic blueprint called RNA, similar to DNA. 
          This single-stranded RNA acts as a molecular message that enables the production of proteins needed for other elements of the virus."
      ),
      tags$li(
        tags$strong("Nucleoproteins"), 
        " Bound to the RNA are nucleoproteins, which help give the virus its structure and enable it to replicate."
      ),
      tags$li(
        tags$strong("Viral envelope (blue):"), 
        " Encapsulating the RNA genome, the viral envelope protects the virus outside a host cell. 
          This outer envelope is made from a layer of lipids, a waxy barrier containing fat molecules. 
          It also anchors the structural proteins needed by the virus to infect cells. 
          Embedded within it are envelope proteins (dark blue dots), which aid the assembly of new virus particles."
      ),
      tags$li(
        tags$strong("Spike proteins (red-orange):"), 
        " The bulbous projections on the outside of the coronavirus are spike proteins. 
          This fringe of proteins gives the virus its crown-like appearance under the microscope (hence the name ", tags$em("corona"), "). 
          The spike proteins act as grappling hooks, allowing the virus to latch onto host cells and infect them."
      )
    ),
    tags$p("Like all viruses, coronaviruses are unable to thrive or reproduce outside of a living host.")
  )
})


plyMesh <- vcgPlyRead(plyPath, updateNormals = TRUE)
output$modelViewer <- renderRglwidget({
  open3d(useNULL = TRUE)
  bg3d(color = "black")
  shade3d(plyMesh, meshColor = "vertices")
  rglwidget()
})

output$domainPlot <- renderPlot({
  plot_spike_domains(features, num_columns = 2)
})
output$mdContent <- renderUI({
  HTML(markdown::markdownToHTML("files/Text/Welcome.Rmd", fragment.only = TRUE))
})
...
observeEvent(input$go_to_country_data_click, {
  removeModal()
  updateTabsetPanel(session, "data", selected = "Country Data")
...
output$cases_comparison_plot <- renderPlot({
  ...
  ggplot(map_data, aes(x = reorder(name_long, `Cases...cumulative.total.per.100000.population`), 
    ...
    scale_fill_manual(values = c("FALSE" = "#EDB273", "TRUE" = "#310F34")) +
    labs(
...
output$deaths_comparison_plot <- renderPlot({
  ...
  ggplot(map_data, aes(x = reorder(name_long, `Deaths...cumulative.total.per.100000.population`), 
                       ...
                       fill = name_long == input$selected_country)) +
    geom_bar(stat = "identity", show.legend = FALSE, alpha = 0.9) +
    scale_fill_manual(values = c("FALSE" = "#f0a5a5", "TRUE" = "#310F34")) +
    labs(
      x = NULL,  # Remove x-axis label for a cleaner look
      y = "Deaths per 100k"
    ) +
...
output$vaccination_comparison_plot <- renderPlot({
  ...
  ggplot(map_data, aes(x = reorder(name_long, percent_vaccinated), 
                       ...
                       fill = name_long == input$selected_country)) +
    geom_bar(stat = "identity", show.legend = FALSE, alpha = 0.9) +
    scale_fill_manual(values = c("FALSE" = "#c8e6c9", "TRUE" = "#310F34")) +
    labs(
      x = NULL,  # Remove x-axis label for a cleaner look
      y = "Vaccinated (% of Population)"
    ) +
...
shinyApp(ui = ui, server = server,
         options = list(host= "127.0.0.1", port = 1234))
